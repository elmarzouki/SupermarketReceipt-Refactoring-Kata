...
from models.product import Product, ProductQuantity
...
class ShoppingCart:
    ...
    def add_item_quantity(self, product: Product, quantity: int) -> None:
        ...
        self._product_quantities[
            product
        ] += quantity  # defaultdict will add defulat 0 value

    # TODO:// we might need to move this logic to another class
    def handle_offers(
        self, receipt: Receipt, offers: Dict[Product, Offer], catalog: Catalog
    ) -> None:
        for product, quantity in self._product_quantities.items():
            if product in offers:
                offer = offers[product]
                unit_price = catalog.unit_price(product)
                discount = self.offer_service.apply_offer(
                    offer, product, quantity, unit_price
                )
                if discount:
